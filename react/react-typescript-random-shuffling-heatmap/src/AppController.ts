...
const AppController = {
  ...
  getHeatmapData: (size: number, runs: number, getPermutation: Function): HeatmapData[] => {
    ...
    for (let i = 0; i < runs; i++) {
      ...
      permutation.forEach((number: number, index: number) => {
  ...
  getState: (): State => {
    ...
    console.log(getHeatmapData(AppController.getOrdered).map(v=>v.y))
    console.log(getHeatmapData(AppController.getOrdered).map(v=>v.x))
    return {
